...
# ! ::: build thermodb for single component
...
# ! ::: check and build thermodb for single component
...
# ! ::: Build components thermodynamic databook
...
# ! ::: Check and Build Components Thermodb
...
# ! ::: check and build mixture thermodb
...
def build_constants_thermodb(
    ...
    **kwargs
    ...
    """
    Build a thermodb containing table-wide constants from explicit config.

    Parameters
    ----------
    reference_config : Mapping[str, Any] | str
        Constants source config or YAML/JSON string config. Each source should
        include ``databook`` and ``table`` keys. Configs generated by
        ``constants_reference_mapper`` are accepted directly.
    custom_reference : CustomReference, optional
        Custom reference dictionary for external references, by default None.
    thermodb_name : str, optional
        Name of the thermodb to build, by default ``"constants"``.
    message : str, optional
        Thermodb description. If omitted, a message is generated from the
        configured source names.
    reference_config_default_check : bool, optional
        Kept for API parity with other config-based builders. Constants configs
        are treated as direct source configs.
    ...
    verbose : bool, optional
        Whether to enable verbose logging, by default False.
    **kwargs
        Additional keyword arguments reserved for API parity with other builders.

    Returns
    -------
    CompBuilder | None
        Built thermodb containing ``TableConstants`` sources, or None when no
        valid constants sources were built.
    ...
    YAML/JSON string configs may optionally wrap the source configs under one
    ...
    try:
        ...
        if not res:
            if verbose:
                logging.warning(
                    "No constants sources were built. Thermodb will not be created."
                )
            return None
        ...
        for source_name, source_value in res.items():
            ...
            if verbose:
                if add_data_res_:
                    logging.info(
                        f"Constants source '{source_name}' added to thermodb '{thermodb_name}'."
                    )
                else:
                    logging.warning(
        ...
        if thermodb_save:
            ...
            if verbose:
                if save_res_:
                    logging.info(
                        f"Constants thermodb '{thermodb_name}' saved to {thermodb_save_path}."
                    )
                else:
                    logging.warning(
                        f"Saving constants thermodb '{thermodb_name}' may have issues."
                    )
        else:
            build_res_ = thermodb_comp.build()
            if verbose:
                if build_res_:
                    ...
                    logging.warning(
                        f"Building constants thermodb '{thermodb_name}' may have issues."
                    )
        ...
        if verbose:
            ...
            elapsed_time = end_time - start_time
            logging.info(
                f"Constants thermodb build completed in {elapsed_time:.2f} seconds."
            )
...
# ! ::: check and build constant thermodb
...
def check_and_build_constants_thermodb(
    ...
    **kwargs
    ...
    """
    Check and build a thermodb containing table-wide constants.
    ...
    constants : str | list[str], optional
        Constant names or symbols to require. If omitted, all valid constants
        sources in the config are built.
    search_mode : Literal['NAME', 'SYMBOL', 'BOTH'], optional
        Lookup mode for requested constants, by default ``"BOTH"``.
    thermodb_name : str, optional
        Name of the thermodb to build, by default ``"constants"``.
    message : str, optional
        Thermodb description. If omitted, a message is generated from the
    ...
    **kwargs
    ...
    -------
    CompBuilder | None
        Built thermodb containing checked ``TableConstants`` sources, or None
        when no configured source passes validation.

    Notes
    -----
    In addition to the checks performed by ``build_constant_thermodb``, this
    method verifies that:

    - the databook exists,
    - the table exists,
    - the table is a constants table,
    - requested constants exist when ``constants`` is provided,
    - configured labels/symbols exist when provided.
    ...
    try:
        if verbose:
            start_time = time.time()
            logging.info("Checking and building constants thermodb...")
        ...
        if not res:
            if verbose:
                logging.warning(
                    "No valid constants sources were built. Thermodb will not be created."
                )
            return None
...
def _normalize_dataset_reference_config(
    ...
    if not isinstance(reference_config, Mapping):
        raise TypeError(
            "reference_config must be a mapping or YAML/JSON string.")
    ...
    if not normalized:
        raise ValueError(
            "reference_config must contain at least one dataset source.")
    return normalized
...
def build_dataset_thermodb(
    ...
    **kwargs,
    ...
    ----------
    reference_config : Union[Mapping[str, Any], str]
        Configuration mapping source names to databook and table selectors.
    custom_reference : Optional[CustomReference], optional
        Custom reference for CSV-backed datasets, by default None.
    thermodb_name : Optional[str], optional
        Name of the ThermoDB to be built, by default None.
    message : Optional[str], optional
        Message describing the ThermoDB, by default None.
    thermodb_save : Optional[bool], optional
        Whether to save the ThermoDB after building, by default False.
    thermodb_save_path : Optional[str], optional
        Path to save the ThermoDB, by default None.
    verbose : bool, optional
        Whether to print verbose logging information, by default False.

    Returns
    -------
    Optional[CompBuilder]
        The built ThermoDB object if successful, otherwise None.
    """
    _ = kwargs

    # SECTION: normalize dataset reference config and initialize source
    configs = _normalize_dataset_reference_config(reference_config)

    # SECTION: set configuration and initialize source
    set_config(AppConfig(include_data=True, build_type="dataset"))

    # SECTION: initialize source and datasets container
    source = init(custom_reference=custom_reference)

    # dataset container
    datasets: Dict[str, TableDataset] = {}

    # SECTION: build datasets from reference config
    ...
    # SECTION: save or build thermodb
    if thermodb_save:
        ...
        if not result.save(filename=thermodb_name, file_path=save_path):
            raise RuntimeError(
                f"Saving dataset thermodb '{thermodb_name}' failed.")
    elif not result.build():
        raise RuntimeError(
            f"Building dataset thermodb '{thermodb_name}' failed.")
...
# ! ::: build component thermodb from reference
...
# ! ::: build interaction ThermoDB from reference content
...
# ! ::: build dataset ThermoDB from reference content
...
def build_dataset_thermodb_from_reference(
    ...
    **kwargs,
    ...
    ----------
    reference_content : str
        The YAML reference content as a string.
    custom_reference : Optional[CustomReference], optional
        Custom reference mapping for CSV-backed references, by default None.
    databook_name : Optional[str], optional
        Name of the databook to restrict discovery, by default None.
    table_name : Optional[str], optional
        Name of the table to restrict discovery, by default None.
    thermodb_name : Optional[str], optional
        Name of the ThermoDB instance, by default None.
    message : Optional[str], optional
        Message to include in the ThermoDB build, by default None.
    thermodb_save : Optional[bool], optional
        Whether to save the ThermoDB instance, by default False.
    thermodb_save_path : Optional[str], optional
        Path to save the ThermoDB instance, by default None.
    verbose : bool, optional
        Whether to enable verbose logging, by default False.
    **kwargs
        Additional keyword arguments passed to the ThermoDB build function.

    Returns
    -------
    Optional[DatasetThermoDB]
        The built DatasetThermoDB instance, or None if no matching tables were found.
    """
    if not isinstance(reference_content, str) or not reference_content.strip():
    ...
    if not configs:
        logger.warning(
            "No matching Dataset tables were found in the reference.")
        return None
