      ...
      TABLES:
        ideal-gas-heat-capacity:
          ...
          EQUATIONS:
            ...
            EQ-2:
              BODY:
                - res['ideal-gas-heat-capacity | Cp_IG | J/mol.K'] = (parms['a0 | a0 | 1'] + parms['a1 | a1 | 1E3']*args['temperature | T | K'] + parms['a2 | a2 | 1E5']*(args['temperature | T | K']**2) + parms['a3 | a3 | 1E8']*(args['temperature | T | K']**3) + parms['a4 | a4 | 1E11']*(args['temperature | T | K']**4))*parms['Universal-Gas-Constant | R | J/mol.K'] - 273.15
              BODY-INTEGRAL: None
        ...
        vapor-pressure:
          ...
          VALUES:
            ...
            - [12,'1,3-Butadiene','C4H6','l',75.572,-4621.9,-8.5323,1.23E-05,2,164.25,6.92E+01,425,4.30E+06,1]
        ...
        NRTL Non-randomness parameters-2:
          ...
          VALUES:
            ...
            - [1,methane|ethanol,methane,CH4,0,0.300492719,0,1.564200272,0,35.05450323,0,4.481683583]
...
# SECTION: verify equations
# NOTE: Heat capacity equation verification
# >> get Cp_IG equation
Cp_IG_equation = component_thermodb.select(
    'CUSTOM-REF-1::ideal-gas-heat-capacity')
# >> print equation
print(f"[bold green]Cp_IG Equation:[/bold green] {Cp_IG_equation}\n")
# >> verify equation by calculating Cp_IG at T=300 K
if not isinstance(Cp_IG_equation, TableEquation):
    raise TypeError("Cp_IG_equation is not of type TableEquation!")

# NOTE: extract equation body
equation_summary = Cp_IG_equation.summary
print(
    equation_summary
)

# NOTE: calculate Cp_IG at T=300 K
T = 300  # K
Cp_IG_value = Cp_IG_equation.cal(
    T=T
)
print(Cp_IG_value)
