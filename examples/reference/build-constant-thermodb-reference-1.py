...
thermodb_constants_: CompBuilder | None = ptdb.build_constant_thermodb(
...
# check
print(f"check: {thermodb_constants_.check()}")
print(f"message: {thermodb_constants_.message}")
print(f"constants sources: {list(thermodb_constants_.check_constants().keys())}")

# ====================================
# SELECT CONSTANTS
# ====================================
custom_constants: TableConstants = thermodb_constants_.select_constant(
    'custom-constants'
)
print(type(custom_constants))
print(custom_constants.data_structure())

# access constants
# ! R (scalar)
print(custom_constants.get_constant('R', message='gas constant'))
# ! dH_rxn (dictionary)
print(custom_constants.get_constant('dH_rxn', message='enthalpy of reaction'))
# ! Xb (string)
print(custom_constants.get_constant('Xb', message='binary parameter'))
# ! X (list)
print(custom_constants.get_constant('X', message='custom constants'))
# ! non-existing constant
print(custom_constants.get_constant('non_existing_constant', strict=False))
...
print(thermodb_constants_.retrieve(
    'custom-constants | dH_rxn',
    message='enthalpy of reaction'
))

custom_constants_2: TableConstants = thermodb_constants_.select_constant(
    'custom-constants-2'
)
...
const1_: TableConstants = thermodb_loaded.select_constant('custom-constants')
print(const1_.get_constant('R', message='loaded gas constant'))
print(thermodb_loaded.retrieve(
    'custom-constants-2 | dG_rxn',
